...
/**
 * Перевірка чи містить текст interrupt keywords (переривання розмови)
 * FIXED (26.10.2025 - 17:40): Додано для функції переривання Atlas під час TTS
 * @param {string} text - Текст для перевірки
 * @returns {boolean} - Чи містить interrupt keyword
 */
export function containsInterruptKeyword(text) {
  if (!text || typeof text !== 'string') {
    return false;
  }
  ...
  // Interrupt keywords - слова що вказують на бажання перервати Atlas
  ...
  // Перевірка на наявність будь-якого interrupt keyword
  for (const keyword of interruptKeywords) {
    if (lowerText.includes(keyword)) {
      logger.info(`🛑 Interrupt keyword detected: "${keyword}" in "${text}"`);
      return true;
    }
  }
