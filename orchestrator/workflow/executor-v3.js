 ...
 */
/**
 * Спрощена функція executeWorkflow
 * Використовує ModeExecutorFactory для вибору режиму виконання
 * 
...
export async function executeWorkflow(userMessage, { logger, wsManager, ttsSyncManager, diContainer, localizationService, res }) {
  ...
  logger.workflow('init', 'mcp', `[${executionId}] Starting workflow execution`, {
    sessionId: session.id,
  ...
  try {
    // Отримати конфігурацію режиму
    const workflowConfig = container.resolve('config').ENV_CONFIG?.workflow || {};
    const engineMode = workflowConfig.engineMode || 'standard';

    logger.system('executor', `[${executionId}] Using mode: ${engineMode}`);

    // Імпортувати ModeExecutorFactory
    const { ModeExecutorFactory } = await import('./modes/mode-executor-factory.js');
    ...
    logger.system('executor', `[${executionId}] Workflow completed`, {
      mode: engineMode,
      duration,
      success: true
    });
    ...
    logger.error('executor', `[${executionId}] Workflow execution failed`, {
      error: error.message,
      stack: error.stack
    });
