...
export class WhisperKeywordDetection extends BaseService {
  constructor(config = {}) {
    ...
    // Додано для контролю циклу помилок
    this.errorCount = 0;
    this.maxRetries = 5;
    this.baseRetryDelay = 1000; // Базова затримка 1 секунда
  }
  ...
  startRecognitionLoop() {
    ...
    this.recordChunk()
      ...
      .finally(() => {
        ...
        if (this.isListening) {
          // Експоненційний відступ при помилках
          const delay = this.errorCount > 0
            ? Math.min(this.baseRetryDelay * Math.pow(2, this.errorCount - 1), 30000)
  ...
  async recordChunk() {
    return new Promise((resolve, reject) => {
      ...
      // Перевірка і очищення попереднього MediaRecorder
      if (this.mediaRecorder) {
        if (this.mediaRecorder.state === 'recording') {
          this.logger.warn('Previous MediaRecorder still recording, stopping it');
          this.mediaRecorder.stop();
        }
        this.mediaRecorder = null;
      }
      ...
      try {
        ...
        this.mediaRecorder.onstop = () => {
          ...
          this.audioChunks = [];
          this.mediaRecorder = null; // Очищення після зупинки
          resolve(audioBlob);
        };

        this.mediaRecorder.onerror = (error) => {
          this.logger.error('MediaRecorder error', null, error);
          this.mediaRecorder = null; // Очищення при помилці
          reject(error);
        };
        ...
        this.logger.error('Failed to create or start MediaRecorder', null, error);
        this.mediaRecorder = null;
        reject(error);
      }
