...
export class ToolHistoryManager {
     ...
     * @param {number} options.maxSize - Maximum history size (default: 100)
     */
    constructor(options = {}) {
        this.history = [];  // Array of ToolCallEntry
        this.maxSize = options.maxSize || 100;
        this.callCounts = new Map();  // tool -> total count
        this.successRates = new Map();  // tool -> { success, total }
        
        logger.system('tool-history', `ToolHistoryManager initialized (maxSize: ${this.maxSize})`);
    }
    ...
    recordToolCall(server, tool, params, success, duration, metadata = {}) {
        ...
        logger.debug('tool-history', `Recorded: ${toolKey} (${success ? '✅' : '❌'}) ${duration}ms`);
    }
