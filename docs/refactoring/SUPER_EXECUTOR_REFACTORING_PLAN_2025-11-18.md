...
- [x] **P2.4**: Інтегрувати WorkflowStateMachine в executeWorkflow – IN PROGRESS (70%)
  ...
  - [ ] **P2.4.4**: Тестування інтеграції (IN PROGRESS - 50%)
    ...
    - [ ] Перевірка state transitions
    - [ ] Перевірка context flow
  ...
  - [ ] **P2.5**: Додати обробку помилок та логування (IN PROGRESS - 10%)
    ...
    - [ ] Обробка невалідних переходів (WorkflowStateMachine)
    - [ ] Обробка помилок handlers (StateHandler)
    - [ ] Централізоване логування (всі компоненти)
    - [ ] Таймаути на критичні стани (WorkflowStateMachine)
...
- [x] **S1**: Executor-v3 розбитий на стадійні функції (Phase 1) – 9 функцій створено + 9 заповнено реальною логікою + Stages 0–8 інтегровано (100% INTEGRATED) + Phase 1.8 testing (30%) + Phase 2 planning (100%) ✅ PHASE 1 COMPLETE | 📋 PHASE 2 READY
- [ ] **S2**: WorkflowStateMachine інтегровано та замінює ручні стани (Phase 2) – IN PROGRESS (P2.1-2.3 DONE, P2.4 80% DONE, P2.4.1-3 DONE)
- [ ] **S3**: OptimizedWorkflowManager використовується для mode/server/tool selection (Phase 3)
- [ ] **S4**: HybridWorkflowExecutor використовується на Stage 2.2 (tools) і, опційно, для TODO items (Phase 4)
- [ ] **S5**: Feature flags дозволяють перемикати режими рушія (Phase 5)
- [ ] **S6**: Старі entrypoints (optimized-executor, окремий hybrid entry) видалені або заархівовані (Phase 6)
- [ ] **S7**: Документація оновлена й зафіксована в `docs/refactoring` та воркфлоу-звітах
